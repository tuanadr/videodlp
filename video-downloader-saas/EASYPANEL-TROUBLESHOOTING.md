...
1. **Kiểm tra tên host Redis**:
   - Đảm bảo biến môi trường `REDIS_HOST` đúng với tên dịch vụ Redis trong Easypanel
   - Thông thường, tên host Redis trong Easypanel có dạng `project-name_service-name`

2. **Tắt Redis tạm thời**:
   - Nếu bạn không cần Redis, hãy xóa các biến môi trường `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD`
...
1. **Thêm route health check rõ ràng**:
   - Đã thêm route `/health` để Easypanel có thể kiểm tra trạng thái dịch vụ

2. **Cải thiện xử lý lỗi Redis**:
   - Đã cập nhật mã nguồn để không gây lỗi nếu không thể kết nối đến Redis

3. **Cải thiện xử lý lỗi cơ sở dữ liệu**:
   - Đã cập nhật mã nguồn để không gây lỗi nếu có vấn đề với cơ sở dữ liệu

4. **Tự động tạo thư mục database**:
   - Đã cập nhật mã nguồn để tự động tạo thư mục database nếu nó không tồn tại
